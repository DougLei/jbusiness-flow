...
public abstract class SqlResolver {
	 ...
	 * @return
	 */
	public abstract Sql parse(String name, DeclaredParameter[] parameters, JSONObject sqlJSON, ReferenceResolver referenceResolver);
	 ...
	 * @return
	 */
	protected final ConditionValidator parseConditionValidator(JSONObject json, ReferenceResolver referenceResolver) {
		if(json == null)
			return ConditionValidator.defaultValidator();
		return ConditionResolver.parse(json.getJSONArray("dynamic"), referenceResolver);
	}
	
	// 解析Function
	private Function parseFunction(JSONObject json, ReferenceResolver referenceResolver) {
		Function function = new Function(ConditionResolver.parse(null, referenceResolver), json.getString("name"));
	...
	// 给子类提供解析必要sql组件的方法
	// 解析Table
	protected Table parseTable(JSONObject json, ReferenceResolver referenceResolver) {
		Table table = new Table(ConditionResolver.parse(null, referenceResolver), json.getString("alias"));
	...
	// 解析column数组
	protected Column[] parseColumns(JSONArray array, ReferenceResolver referenceResolver) {
	...
	// 解析Value
	protected Value parseValue(JSONObject json, ReferenceResolver referenceResolver) {
		Value value = new Value(parseConditionValidator(json, referenceResolver));
		setValue(json, value, referenceResolver);
		return value;
	}
	// 给value中设置值
	private void setValue(JSONObject json, Value value, ReferenceResolver referenceResolver) {
		Object object;
	...
	// 解析select
	private Select parseSelect(JSONObject json, ReferenceResolver referenceResolver) {
	...
	// 解析condition中right部分
	private Value[] parseRights(JSONArray array, ReferenceResolver referenceResolver) {
		if(CollectionUtil.isEmpty(array))
	...
	// 解析group by
	private GroupBy[] parseGroupBys(JSONArray array, ReferenceResolver referenceResolver) {
		if(CollectionUtil.isEmpty(array))
			return null;
	...
	// 解析order by
	private OrderBy[] parseOrderBys(JSONArray array, ReferenceResolver referenceResolver) {
		if(CollectionUtil.isEmpty(array))
			return null;
	...
	// 设置group by的信息
	private void setGroupByInfo(GroupBy groupBy, JSONObject json, ReferenceResolver referenceResolver) {
